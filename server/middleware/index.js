...
export const cors = function (req, res, next) {
    ...
    let allowDomain = ['http://apidoc.hefantv.com', 'http://testapidoc.hefantv.com', 'http://localhost.hefantv.com:3100']
 ...
 * @param {*} next 
 */
export var auth = async function (req, res, next) {
    ...
    Log.debug(`cookie--> HEFAN_TOKEN ${req.cookies['HEFAN_TOKEN']}`)

    HEFAN_TOKEN = req.cookies['HEFAN_TOKEN']
    ...
    if(!url_arr.includes(req.baseUrl + req.path)) {
        if (!_.isPlainObject(params)) {
            ...
            //自动登录
            if((req.baseUrl + req.path) == '/api/autologin' && HEFAN_TOKEN){
                try {
                    //认证系统获得用户数据
                    ssoUser = await axios.get(AUTH_HOST + '/api/get_user_info.do', { params: {token: HEFAN_TOKEN} })
                ...
                if(ssoUser.id){
                    //查询存在ssouser字段的用户
                    let userdos = await Entity.find(Model.user, {ssoUser: {$exists: true}} ).catch((e) => {
                    ...
                    if(!_data){
                        //是否能关联用户
                        let userdos = await Entity.find(Model.user, {username: ssoUser.username} ).catch((e) => {
                            resJson = Serrors.findError()
                        })
                    ...
                    if(_data){
                        ...
                        token = jwt.sign({
                            ...
                            data: _data
                        }, Constant.secret, {expiresIn: '1720h'}); //, {expiresIn: '720h'}
...
export const authority = async function (req, res, next) {
    let params = Params.tokenValidate(req),
        url_arr = ['/api/delInterfaces','/api/updateAddInterfaces','/api/delStateCode','/api/getStateCodes'],
        resJson,
        projectDoc,
        isCooperGroup = -1
    ...
    if(url_arr.includes(req.baseUrl + req.path)) {
        if (params && params.projectId) {
            projectDoc = await Entity.findById(Model.project, params.projectId).catch(e => {
                resJson = Serror.findError('project 查询失败')
            })

            if (projectDoc && projectDoc.cooperGroup.length > 0) {
                isCooperGroup = projectDoc.cooperGroup.findIndex((item,index)=> item.userId === domain.userId)
            }

            if (!(isCooperGroup > -1 || parseInt(domain.authority) > 2)) {
                resJson = Serror.Unauthorized('没有权限')
            }
        }else {
            resJson = Serrors.paramsError()
        }
