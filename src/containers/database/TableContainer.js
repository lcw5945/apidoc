...
export default class TableContainer extends React.Component {
    ...
    this.state = {
      ...
      tableColumns: [
        //表格的结构数据
        {
          ...
          render: a => {
            return a === 1 || a === "1" ? (
              <Icon type="key" style={{ color: "#f48932" }} />
            ) : (
              ""
            );
          }
        ...
        },
        {
          ...
          render: a => {
            return a ? (
              <Popover content={a} trigger="click">
                <span className="descBtn descDetailBtn">查看</span>
              </Popover>
            ) : (
              <span className="descBtn">无</span>
            );
          }
        },
        {
          ...
          render: data => {
            return (
              <div style={{ display: this.state.isAdmin ? "block" : "none" }}>
                <Button icon="edit" onClick={this.updateField.bind(this, data)}>
                  修改
                </Button>
                <Popconfirm
                  title="确定删除此字段吗？"
                  okText="确定"
                  cancelText="取消"
                  onConfirm={this.fieldDel.bind(this, data)}
                >
   ...
   * */
  getGroupList() {
    const queryData = (this.state.queryData = Utils.parseUrlToData(
  ...
  render() {
    ...
    this.state.isAdmin =
      this.getDatabaseAdmin() == this.props.user.userId ||
      this.props.user.authority > 2
        ? true
        : false;
    ...
    if (items[0] && tableGroupList.length > 0) {
      groupListId =
        groupListId == "-1" ? tableGroupList[0]._id : queryData.groupId;
      ...
      <div>
        ...
        <div className="tableList" style={{ marginLeft: groupCmp.width }}>
          <div className="buttons">
            <Button
              type="primary"
              icon="plus"
              onClick={this.updateField.bind(this, null)}
              style={{ display: this.state.isAdmin ? "inline-block" : "none" }}
            >
              添加字段
            </Button>
            <span className="allCounts">
              字段总数：{this.state.fieldsLength}
            </span>
          </div>
          {items &&
            (() => {
              ...
              return (
                <Table dataSource={items} columns={this.state.tableColumns} />
              );
            })()}
        </div>
        ...
        >
          <Form className="fieldForm">
            <FormItem {...formItemLayout} label="所属表:" hasFeedback>
            ...
            <FormItem {...formItemLayout} label="字段名:" hasFeedback>
            ...
            <FormItem {...formItemLayout} label="字段长度" hasFeedback>
              {getFieldDecorator("length", {
                rules: [
                  {
                    required: true,
                    message: "字段长度为数字且不能为空!",
                    whitespace: true
                  }
                ]
              })(<Input type="number" placeholder="字段长度" />)}
            </FormItem>
            <FormItem {...formItemLayout} label="主键" hasFeedback>
            ...
            <FormItem {...formItemLayout} label="必填" hasFeedback>
            ...
            <FormItem label="字段描述" {...formItemLayout}>
