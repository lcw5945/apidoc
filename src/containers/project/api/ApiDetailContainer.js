...
export default class ApiDetailContainer extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            ...
            returnDataColumns: [
                {
                    ...
                    render: (text, record, index) => (
                        <div className="request">
                            <div className="request-index">{index + 1}</div>
                            {
                                record.include === 0 ? <div className="request-value gray">非必含</div> :
                ...
                {
                    title: '参数',
                    dataIndex: 'returnDataParameter',
                    render: (text, record) => (
                        <div className="parameter">
                            <div className="parameter-key">{record.returnDataKey}</div>
                            <div className="parameter-dec">{record.returnDataDes}</div>
                        </div>
                    )
                },
                {
                    title: '值可能性',
                    dataIndex: 'returnDataKey',
                    render: (text, record) => (
                        <div className="key">
                            {
                                record.value.map((valueCont, index) =>
                ...
                <ProjectSubnav />
                {
                    interfaces && projects && (() => {
                        if (interfaces.hasOwnProperty(queryData.projectId)) {
                            ...
                            // 为Table组件添加key值
                            if (item.params) {
                                dataSource = this.cloneArr(item.params, dataSource);
                                dataSource.forEach((param, index) => {
                                    param['forKey'] = index;
                                })
                            }
                            if (item.returnData) {
                                item.returnData.forEach((param, index) => {
                                    param['forKey'] = index;
                                })
                            }
                            if (item.apiJson) {
                            ...
                            <div className="apiDetail" style={{'marginLeft': groupCmp.width}}>
                                <div className="buttons clearfix">
                                    ...
                                    <Button icon="caret-right"
                                            onClick={this.goToTest.bind(this)}>测试</Button>
                                ...
                                <div className="apiDetail-main">
                                    <div className="top-show clearfix">
                                        ...
                                        <div
                                            className="top-show-time">{ Utils.formatDate(new Date(parseInt(item.update))) }</div>
                                    </div>
                                    ...
                                    <div className="requestTable">
                                        <Table dataSource={item.returnData} columns={this.state.returnDataColumns}
                                               size="small" pagination={false} rowKey="forKey"/>
                                    </div>
