...
export default class ApiContainer extends React.Component {
                                ...
                                return (
                                    <div>
                                        <div style={{display: String(obj.groupId) !== '-2' ? 'block' : 'none'}}>
                                            <Button icon="copy" onClick={this.setCopyDate.bind(this,obj)}>拷贝</Button>
                                            <Button icon="edit"
    ...
    goInterfaceDetail(record, index, event) {
        ...
        this.props.history.push(`/project/api/details?projectId=${record.projectId}&groupId=-1&apiId=${record._id}`)
    }
    ...
    getUpdateDialog(projectId, apiId, type, event) {
        ...
        event.preventDefault();
        if (type === 'edit') {
            this.props.history.push(`/project/api/edit?projectId=${projectId}&groupId=-1&apiId=${apiId}`)
        } else {
            let opt = {
                id: apiId,
                groupId: -1,
            ...
            <div className='api-container'>
                {
                    interfaces && projects && (() => {
                        ...
                        if (interfaces.hasOwnProperty(queryData.projectId)) {
                            ...
                            if (searchConent) {
                                itemList = _.filter(itemList, function (_data) {
                                    if (_data.featureName.indexOf(searchConent) > -1 || _data.URI.indexOf(searchConent) > -1) {
                                        return true;
                                    }
                                });
                            }
                        ...
                        return (<Table dataSource={ dataSource} columns={ this.state.tableColumns}
